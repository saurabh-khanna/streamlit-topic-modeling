...
DATASETS = {
    "Elon Musk Tweets 2015-2020": {
        ...
        "description": (
            "I leverage Elon Musk's tweets from the last 5 years using twint library."
        ),
    },
    # 'Airline Tweets': {
...
def nmf_options():
    return {
        ...
        "kappa": st.number_input(
            "𝜅", min_value=0.0, value=1.0, help="Gradient descent step size."
        ),
        "minimum_probability": st.number_input(
            "Minimum Probability",
            min_value=0.0,
        ...
        "normalize": st.selectbox(
            "Normalize", (True, False, None), help="Whether to normalize the result."
        ),
    }
...
def denoise_docs(texts_df: pd.DataFrame, text_column: str):
    ...
    docs = [
        [
            w
            for w in simple_preprocess(doc, deacc=True)
...
def perplexity_section():
    with st.spinner("Calculating Perplexity ..."):
        perplexity = calculate_perplexity(
            st.session_state.model, st.session_state.corpus
        )
    key = "previous_perplexity"
    delta = (
        f"{perplexity - st.session_state[key]:.4}" if key in st.session_state else None
    )
    st.metric(
        label="Perplexity",
    ...
    st.latex(
        r"Perplexity = \exp\left(-\frac{\sum_d \log(p(w_d|\Phi, \alpha))}{N}\right)"
    )
...
def coherence_section():
    with st.spinner("Calculating Coherence Score ..."):
        coherence = calculate_coherence(
            st.session_state.model, st.session_state.corpus, "u_mass"
        )
    key = "previous_coherence_model_value"
    delta = (
        f"{coherence - st.session_state[key]:.4f}" if key in st.session_state else None
    )
    st.metric(label="Coherence Score", value=f"{coherence:.4f}", delta=delta)
    st.session_state[key] = coherence
    st.markdown(
        "Viz., http://svn.aksw.org/papers/2015/WSDM_Topic_Evaluation/public.pdf"
    )
    st.latex(
...
if __name__ == "__main__":
    ...
    with preprocessing_options:
        ...
        ngrams = st.selectbox(
            "N-grams", [None, "bigrams", "trigams"], help="TODO ..."
        )  # TODO ...
    ...
    with st.expander("What is topic modeling?"):
        ...
        st.markdown(
            "This factorization makes the methods much more efficient than directly characterizing documents "
            "in term of words."
        )
        st.markdown(
            "More information on LDA and NMF can be found at "
    ...
    st.header("Datasets")
    st.markdown("Preloaded an example dataset to illustrate.")
    selected_dataset = st.selectbox(
        "Dataset", [None, *sorted(list(DATASETS.keys()))], on_change=clear_session_state
    )
    ...
    with st.expander("Frequency Sized Corpus Wordcloud"):
        ...
        st.image(
            wc.to_image(),
            caption="Dataset Wordcloud (Not A Topic Model)",
    ...
    with st.expander("Document Word Count Distribution"):
        ...
        fig, ax = plt.subplots()
        sns.histplot(
            data=pd.DataFrame(len_docs, columns=["Words In Document"]),
            discrete=True,
            ax=ax,
        )
        st.pyplot(fig)
    ...
    model_key = st.sidebar.selectbox(
        "Model", [None, *list(MODELS.keys())], on_change=clear_session_state
    )
    ...
    if train_model_clicked:
        with st.spinner("Training Model ..."):
            id2word, corpus, model = train_model(
                docs, MODELS[model_key]["class"], **st.session_state.model_kwargs
            )
    ...
    with st.expander("Top N Topic Keywords Wordclouds"):
        ...
        for index, topic in enumerate(topics):
            ...
            with cols[index % 3]:
                ...
                st.image(
                    wc.to_image(), caption=f"Topic #{index}", use_column_width=True
                )
    ...
    with st.expander("Topic Highlighted Sentences"):
        ...
        for index, row in sample.iterrows():
            ...
            for token in row[text_column].split():
                if st.session_state.id2word.token2id.get(token) is None:
                    html_elements.append(
                        f'<span style="text-decoration:line-through;">{token}</span>'
                    )
                else:
                    term_topics = st.session_state.model.get_term_topics(
                        token, minimum_probability=0
                    )
                    topic_probabilities = [term_topic[1] for term_topic in term_topics]
                    max_topic_probability = (
                        max(topic_probabilities) if topic_probabilities else 0
                    )
            ...
            st.markdown(
                f'Document #{index}: {" ".join(html_elements)}', unsafe_allow_html=True
            )
    ...
    with st.expander("Low Dimensional Projections"):
        with st.form("projections-form"):
            ...
            projection = left_column.selectbox(
                "Projection", ["PCA", "T-SNE", "UMAP"], help="TODO ..."
            )
        ...
        if generate_projection_clicked:
            topic_weights = []
            for index, topic_weight in enumerate(
                st.session_state.model[st.session_state.corpus]
            ):
            ...
            with st.spinner("Training Projection"):
                projections = train_projection(
                    projection, n_components, df.drop(columns=["text"])
                )
    ...
    if hasattr(
    ...
    ):  # gensim Nmf has no 'inference' attribute so pyLDAvis fails
        if st.button("Generate pyLDAvis"):
            ...
            with st.expander("pyLDAvis", expanded=True):
                st.markdown(
                    "pyLDAvis is designed to help users interpret the topics in a topic model that has been "
